const officerData = [
    {
        name: "Alya Mansoor",
        title: "President",
        image: '',
        linkedin: '',
        email: '',
    },
    {
        name: "Sidrah Rashad",
        title: "Vice President",
        image: '',
        linkedin: '',
        email: '',
    },
    {
        name: "Gabrielle Tobias",
        title: "Treasurer",
        image: '',
        linkedin: '',
        email: '',
    },
    {
        name: "Rushil Sivaiah",
        title: "Secretary",
        image: "",
        linkedin: '',
    },
    {
        name: "Dyanada Gavande",
        title: "Director of Tutoring",
        image: '', // Please provide the image if available
        linkedin: '',
        email: '',
    },
    {
        name: "Deston Muo",
        title: "Head of Web Development",
        image: '',
        linkedin: '',
        email: '',
    },
    {
        name: "Tian Wang",
        title: "Director of Business",
        image: '', 
        linkedin: '',
        email: '',
    },
    {
        name: "James Dao",
        title: "Director of CI",
        image: "", // Please provide the image if available
        linkedin: '',
        email: '',
    },
    {
        name: "Muneeb Shahid",
        title: "Head of Director of Engineering",
        image: '', // Please provide the image if available
        linkedin: '',
        email: '',
    },
    {
        name: "Darras Muntasir Rahman",
        title: "PES Director",
        image: '', // Please provide the image if available
        linkedin: '',
        email: '',
    },
    {
        name: "Ana Wise",
        title: "Director of Robotics",
        image: '', // Please provide the image if available
        linkedin: '',
        email: '',
    },
    {
        name: "Brandon Pruett",
        title: "Director of Robotics",
        image: '', // Please provide the image if available
        linkedin: '',
        email: '',
    },
    {
        name: "Bhavya Chandran",
        title: "Director of MTTS/APS",
        image: "", // Please provide the image if available
        linkedin: '',
        email: '',
    },
    {
        name: "Soham Samantaray",
        title: "MTTS/APS Project Supervisor",
        image: "", // Please provide the image if available
        linkedin: '',
        email: '',
    },
    {
        name: "Travis Stanger",
        title: "MTTS/APS Project Supervisor",
        image: "", // Please provide the image if available
        linkedin: '',
        email: '',
    },
    {
        name: "Arjun Agravat",
        title: "CI Project Supervisor",
        image: "", // Please provide the image if available
        linkedin: '',
        email: '',
    },
    {
        name: "Rishi Villa",
        title: "CI Project Supervisor",
        image: "", // Please provide the image if available
        linkedin: '',
        email: '',
    },
    {
        name: "Anna Xie",
        title: "Head Director of Marketing",
        image: "", // Please provide the image if available
        linkedin: '',
        email: '',
    },
    {
        name: "Quynh Do",
        title: "Social Media Manager",
        image: "", // Please provide the image if available
        linkedin: '',
        email: '',
    }
];

export default officerData;
